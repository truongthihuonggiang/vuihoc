...
1,c,e)},showImage:function(c,e,a){this.setNextIndex(c);this.setupImages();this.transition(e,a)},finishedLoading:function(){var c=this;this.container.css({width:this.width+"px",height:this.height+"px"});this.imageContainer.removeClass("loading");if(this.options.pagination){this.pagination=b('<ul class="pagination"></ul>').css({margin:"0px",padding:"0px"});this.pagination.bind("click",function(a){a.preventDefault();c.showImage(b(a.target).data("index"))});b(this.images).each(function(a){var d=
...
/*---------------------------------------------------CONTROLL-NEXT-PRE-----------------------------------------------*/
this.pagination.height():0)+"px"});if(this.options.controls){var e={width:"50px",height:"50px",padding:"5px 0 0 12px","font-size":"40px","line-height":"0.9em",color:"rgba(255, 255, 255,0.8)","text-decoration":"none",background:"rgba(0,0,0,0.5)",position:"absolute","z-index":110};this.nextButton=b('<a href="#"><i class="fas fa-angle-right"></i></a>').css(e).css3({"border-radius":"50%","padding":"5px 0 0 15px "}).appendTo(this.surface).bind("click",function(a){a.preventDefault();c.next()});this.prevButton=b('<a href="#"><i class="fas fa-angle-left"></i></a>').css(e).css3({"border-radius":"50%"}).appendTo(this.surface).bind("click",
function(a){a.preventDefault();c.prev()});e=(this.height-this.nextButton.height())/2;this.nextButton.css({top:"370px",right:"60px"});this.prevButton.css({top:"370px",left:"60px"})}if(this.options.captions)this.captionBar=b('<div class="caption"></div>').css({background:"rgba(0,0,0,0.6)",color:"#FFF","font-size":"16px","font-family":"helvetica, arial, sans-serif","text-decoration":"none",padding:"1em 1em",opacity:0,position:"absolute","z-index":110,width:"100%",bottom:0}).css3({"transition-property":"opacity",
